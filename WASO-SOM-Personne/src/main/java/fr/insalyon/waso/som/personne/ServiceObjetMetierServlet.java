    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("getListePersonne".equals(som)) {
                ...
                String[] idsPersonneParametre = request.getParameterValues("id-personne");
                if (idsPersonneParametre == null) {
                    throw new ServiceException("Paramètres incomplets");
                }
                  // Create a list to store the parsed integers
                List<Integer> idsPersonne = new ArrayList<>();

                // Iterate over the array of string parameters and parse them into integers
                for (String id : idsPersonneParametre) {
                    idsPersonne.add(Integer.parseInt(id));
                }

                service.getPersonneParId(idsPersonne);
