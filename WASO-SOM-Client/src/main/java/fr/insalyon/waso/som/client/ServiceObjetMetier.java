        ...
        try {
            ...
            if(ville==null) {
                    listeClients = this.dBConnection.launchQuery(
                        "SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM Client WHERE LOWER(Denomination) LIKE ?", 
                        "%" +  denomination.toLowerCase() + "%"
                    );
                
            } else {
                listeClients = this.dBConnection.launchQuery(
                    "SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM Client WHERE LOWER(Denomination) LIKE ? AND LOWER(Ville) LIKE ?", 
                    "%" + denomination.toLowerCase() + "%", 
                    "%" + ville.toLowerCase() + "%"
                );
