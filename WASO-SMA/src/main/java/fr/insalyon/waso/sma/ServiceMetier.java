...
public class ServiceMetier {
    ...
    public void rechercherClientParDenomination(String denominationParametre, String villeParametre) throws ServiceException {
    try {

        // 1. Obtenir la liste des Clients par dénomination et ville
        JsonObject clientContainer = null;
        try {
            clientContainer = this.jsonHttpClient.post(
                    this.somClientUrl,
                    new JsonHttpClient.Parameter("SOM", "rechercherClientParDenomination"),
                    new JsonHttpClient.Parameter("denomination", denominationParametre),
                    new JsonHttpClient.Parameter("ville", villeParametre)
            );
        } catch (ServiceIOException ex) {
        ...
        throw JsonServletHelper.ServiceMetierExecutionException("getListeClientParDenomination", ex);
    }
}
